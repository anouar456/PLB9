"""
data_processing.py - Prétraitement des données solaires pour la prédiction des pics de puissance.
"""
...
def load_data():
    ...
    for file in csv_files:
        ...
        # Convertir DATE_TIME avec format flexible
...
def handle_missing_values(df):
    ...
    print(f"[INFO] {missing} valeurs manquantes détectées")
    ...
    print(f"[INFO] Valeurs manquantes après traitement : {df.isnull().sum().sum()} ✅")
    return df
...
def optimize_memory(df):
    if len(df) == 0:
        print("[WARNING] DataFrame vide — optimisation ignorée")
        return df
...
# ============================================
# 4. COLONNE CIBLE (DÉTECTION DES PICS)
# ============================================
...
def split_features_target(df, target_col='power_peak'):
    ...
    y = df[target_col]
    print(f"[INFO] Features : {X.shape[1]} colonnes — {list(X.columns)}")
    return X, y
...
def save_processed_data(X_train, X_test, y_train, y_test, label_encoders, scaler, plant_id):
    ...
    train_df = X_train.copy(); train_df['power_peak'] = y_train.values
    test_df  = X_test.copy();  test_df['power_peak']  = y_test.values
    ...
    print(f"[INFO] Données sauvegardées dans {processed_dir} ✅")
    ...
    print(f"[INFO] Encoders/Scaler sauvegardés dans {MODELS_DIR} ✅")
...
def preprocess_pipeline(plant_id='1', power_col='DC_POWER', threshold_pct=80):
    ...
    if plant_id not in data_dict:
        print(f"[WARNING] Centrale {plant_id} non trouvée")
    ...
    if len(df) == 0:
        print("[ERROR] DataFrame vide — arrêt du pipeline")
...
if __name__ == "__main__":
    for plant in ['1', '2']:
        print(f"\n{'='*60}")
        print(f"🏭 TRAITEMENT CENTRALE {plant}")
        print(f"{'='*60}")
        result = preprocess_pipeline(plant_id=plant)
